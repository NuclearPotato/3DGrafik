...
window.onload = function init() {
    ...
    minicanvas = document.getElementById( "gl-minicanvas" );
    
    gl = WebGLUtils.setupWebGL( canvas );
    minigl = WebGLUtils.setupWebGL( minicanvas );
    if ( !gl ) { alert( "WebGL isn't available" ); }
    if ( !minigl ) { alert( "WebGL isn't available" ); }
    
    gl.viewport( 0, 0, canvas.width, canvas.height );
    gl.clearColor( 0.8, 0.8, 0.8, 1.0 );
    gl.clear( gl.COLOR_BUFFER_BIT );
	
	minigl.viewport( 0, 0, minicanvas.width, minicanvas.height );
    minigl.clearColor( 0.0, 0.0, 0.0, 1.0 );
    minigl.clear( gl.COLOR_BUFFER_BIT );
	...
	document.getElementById("slideG").onchange = function(event) { 
		g = event.target.value; 
		color = vec4(r, g, b, 1.0);
		updateColor(r,g,b);
	};
	document.getElementById("slideB").onchange = function(event) { 
